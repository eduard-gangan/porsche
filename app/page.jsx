...
const page = () => {
  ...
  const [position, setposition] = useState([9, -2, -15])
    ...
    gsap.to(model.current, {
      scrollTrigger: {
        ...
        onUpdate: ({progress, getVelocity}) => {
          ...
          setposition([-14 * progress + 9, -2, -15])
    ...
    <main ref={main} className="min-h-screen bg-zinc-900 bg-[url('/bg.svg')] overflow-x-hidden">
      ...
      <section id="section1" className="min-h-screen  grid grid-cols-5 place-content-center relative">
        ...
        <div className="col-start-4 col-span-2 font-bold max-w-lg w-fit relative bg-zinc-800 rounded-3xl shadow-md spec">
          <h2 className="text-2xl xsh:text-4xl text-zinc-200 py-4 px-8 xsh:p-10 from-red-600 to-red-900 bg-blend-overlay bg-[url('/dots.svg'),_theme('backgroundImage.gradient-to-br')] rounded-[inherit]">Feel the power</h2>
          <div className="text-xl xsh:text-3xl text-zinc-500 p-5 xsh:p-8 grid gap-3">
      ...
      <section id="section2" className="h-screen min-h-[600px] relative">
        <div className="h-2/3 w-full bg-[url('/wavy.svg')] absolute bottom-0 left-0 origin-bottom skew-y-[20deg]"></div>
        <div className="absolute text-base xsh:text-xl text-zinc-400 bg-zinc-800 bg-[url('/carbon.svg')] rounded-3xl px-6 py-4 xsh:py-8 z-20 spec shadow-xl shadow-[hsl(0,76%,5%)] top-32 left-32">
          <h3 className="text-zinc-300 font-bold">Rear-mounted engine</h3>
          for improved acceleration and traction
        </div>
        <div className="absolute text-base xsh:text-xl text-zinc-400 bg-zinc-800 bg-[url('/carbon.svg')] rounded-3xl px-6 py-4 xsh:py-8 z-20 spec shadow-xl shadow-[hsl(0,76%,5%)] top-1/2 -translate-y-1/2 right-0 xsh:top-32 xsh:translate-y-0 xsh:left-1/2">
          <h3 className="text-zinc-300 font-bold">Carbon Fiber Everywhere</h3>
          to reduce as much weight as possible
        </div>
        <div className="absolute text-base xsh:text-xl text-zinc-400 bg-zinc-800 bg-[url('/carbon.svg')] rounded-3xl px-6 py-4 xsh:py-8 z-20 spec shadow-xl shadow-[hsl(0,76%,5%)] bottom-16 left-1/4">
      ...
      <section id="section3" className="h-screen min-h-[600px] relative">
        <div className="absolute text-lg xsh:text-xl text-zinc-400 bg-zinc-800 bg-[url('/carbon.svg')] rounded-3xl px-6 py-4 xsh:py-8 z-20 spec shadow-xl shadow-[hsl(0,76%,5%)] top-48 right-16">
          <h3 className="text-zinc-300 font-bold">Adjustable Wing</h3>
          860kg of downforce at 285km/h
        </div>
        <div className="absolute text-lg xsh:text-xl text-zinc-400 bg-zinc-800 bg-[url('/carbon.svg')] rounded-3xl px-6 py-4 xsh:py-8 z-20 spec shadow-xl shadow-[hsl(0,76%,5%)] bottom-32 right-16">
